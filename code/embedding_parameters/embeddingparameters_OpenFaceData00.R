...
timeserie_poseTx <- temp$pose_Tx
  ...
  EE <- data.table()
  for (tau_i in taus){
      Et<- as.data.table(cao97sub(timeserie_poseTx,maxd,tau_i) )
  ...
  funcc <-function(x) {list( selected_participant_number[pNN_k] )}
  EE[,c("participant"):=funcc(), ]
  setcolorder(EE, c(5,1:4))
  E <- rbind(E, EE )
