...
dataTable <- dataTable[,c(19:433):=NULL] # remove columns
...
#   geom_line( aes(x=frame,y=pose_Tx, color='blue'), size=0.7)+
...
timeserie_poseTx <- temp$pose_Tx

tau.ami<-timeLag(timeserie_poseTx, technique = "ami",selection.method = "first.e.decay",lag.max = 100, do.plot = F)
ami <-  rbind( ami, tau.ami )


emb.dim = estimateEmbeddingDim( timeserie_poseTx,
                                time.lag = tau.ami,
                                max.embedding.dim = 15,
                                threshold=0.95, #0.95 is the default
                                do.plot = F)

edim <-  rbind( edim, emb.dim )


p <-  rbind( p,  selected_participant_number[pNN_k] )

}


EMP <- cbind(p,edim,ami)
colnames(EMP) <- c('participant',"min_m","min_tau")
...
################################################################################
####  (6)  Writing Data
write.table(EMP, "EmbeddedParameters.dt", row.name=FALSE)
